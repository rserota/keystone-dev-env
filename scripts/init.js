...
try {
	exec("docker-compose exec -T web npm run create-tables", {stdio: "inherit"})
}catch(e){console.log("create-tables did not run")}
console.log('Running any new migrations')
exec("docker-compose exec -T web npx knex migrate:latest", {stdio: "inherit"})
